#include <iostream>
#include <random>
#include <algorithm>
#define N 100000
using namespace std;

// 快速排序
void quickSort(vector<int>& vec, unsigned long long low = 0, unsigned long long high = -1) {
    if (high == -1) high = vec.size() - 1;
    if (high <= low) return;

    unsigned long long random_idx = low + rand() % (high - low + 1);
    swap(vec[low], vec[random_idx]);
    int pivot = vec[low];

    unsigned long long i = low, j = high;
    while (true) {
        while (vec[i] < pivot) i++;
        while (vec[j] > pivot) j--;
        if (i >= j) break;
        swap(vec[i++], vec[j--]);
    }
    quickSort(vec, low, j);
    quickSort(vec, j + 1, high);
}

int main(){
    system("chcp 65001");
    random_device rd;
    mt19937 gen(rd());
    uniform_int_distribution<int> dis(0, 100000); // 随机数生成种子

    vector<int> vec1;
    for(int i = 0; i < N; i++) vec1.push_back(dis(gen));
    vector<int> vec2 = vec1;
    cout << "成功生成了 10 万个数字" << endl;

    auto start = clock();
    quickSort(vec1);
    auto end = clock();
    cout << "前十个最小数为：" << endl;
    for(int i = 0; i < 10; i++) cout << vec1[i] << " ";
    cout << endl << "运行时间为" << end - start << "ms" << endl;

    start = clock();
    sort(vec2.begin(), vec2.end());
    end = clock();
    cout << "前十个最小数为：" << endl;
    for(int i = 0; i < 10; i++) cout << vec2[i] << " ";
    cout << endl << "运行时间为" << end - start << "ms" << endl;

    return EXIT_SUCCESS;
}